...
class Department(models.Model):
    ...
    def authenticate(self, faculty_key):
        return self.facultyKey == faculty_key
...
class FacultyKeyForm(forms.Form):
    def __init__(self, *args, **kwargs):
        super(FacultyKeyForm, self).__init__(*args, **kwargs)
        self.fields['faculty_key'] = forms.ChoiceField(
            label='Faculty Key',
            choices=self.get_faculty_key_choices(),
            widget=forms.Select(attrs={'class': 'form-control'})
        )
class StudnetKeyForm(forms.Form):
    def __init__(self, *args, **kwargs):
        super(StudnetKeyForm, self).__init__(*args, **kwargs)
        self.fields['student_key'] = forms.ChoiceField(
            label='Student Key',
            choices=self.get_student_key_choices(),
            widget=forms.Select(attrs={'class': 'form-control'})
        )

def get_faculty_key_choices(self):
        courses = Course.objects.all()
        choices = []
        for course in courses:
            faculty = Faculty.objects.filter(faculty_id=course.faculty_id).first()
            if faculty:
                choice_label = f"Course: {course.name}, Faculty Name: {faculty.name}, Faculty Key: {course.facultyKey}"
                choices.append((course.name, choice_label))
        return choices


def get_student_key_choices(self):
    courses = Course.objects.all()
    choices = []
    for course in courses:
        student = Student.objects.filter(course_id=course.code).first()
        if student:
            choice_label = f"Course: {course.name}, Student Name: {student.name}, Student Key: {course.studentKey}"
            choices.append((course.name, choice_label))
    return choices
